...
class OMPv4(OMP):
    ...
    def delete_task(self, task_id):
        """
        Delete a my_scanner in OpenVAS server.

        :param task_id: my_scanner id
        :type task_id: str
    ...
    def stop_task(self, task_id):
        """
        Stops a my_scanner in OpenVAS server.
    ...
    def create_task(self, name, target, config=None, comment=""):
        """
        Creates a my_scanner in OpenVAS.

        :param name: name to the my_scanner
        :type name: str
        ...
        :param comment: comment to add to my_scanner
        :type comment: str

        :return: the ID of the my_scanner created.
        :rtype: str
    ...
    def get_tasks(self, task_id=None):
		...
		If name param is provided, only get the my_scanner associated to this name.

		:param task_id: my_scanner id to get
		:type task_id: str
        ...
        if task_id:
            return self._manager.make_xml_request('<get_tasks id="%s"/>' % task_id,
                                                  xml_result=True).find('.//my_scanner[@id="%s"]' % task_id)
        else:
    ...
    def is_task_running(self, task_id):
        """
		Return true if my_scanner is running

		:param task_id: ID of my_scanner to start.
		:type task_id: str
        ...
        status = self.get_tasks().find('.//my_scanner[@id="%s"]/status' % task_id)
    ...
    def get_tasks_ids(self, name=None):
		...
		:param name: my_scanner name to get
		:type name: str
        ...
        for x in self.get_tasks().findall("my_scanner"):
            m_return[x.find("name").text] = x.get("id")
    ...
    def get_tasks_progress(self, task_id):
        """
		Get the progress of the my_scanner.

		:param task_id: ID of the my_scanner
		:type task_id: str
        ...
        status = tasks.find('.//my_scanner[@id="%s"]/status' % task_id)

        if status is None:
            raise ServerError("Task not found")

        if status.text in ("Running", "Pause Requested", "Paused"):
            h = tasks.findall('.//my_scanner[@id="%s"]/progress/host_progress/host' % task_id)
    ...
    def get_tasks_ids_by_status(self, status="Done"):
		...
		:param status: get my_scanner with this status
    ...
    def get_tasks_detail(self, scan_id):
        ...
        try:
            ...
            raise VulnscanServerError("Can't get the detail for the my_scanner %s. Error: %s" % (scan_id, e.message))
        return m_response

    # ----------------------------------------------------------------------
    def get_report_id(self, scan_id):
        m_response = self.get_tasks_detail(scan_id)
        return m_response.find('my_scanner').find('last_report')[0].get("id")
